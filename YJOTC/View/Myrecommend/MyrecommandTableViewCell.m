...
- (void)refreshWithModel:(reListModel *)model{
    NSString *dataString = [self timestampSwitchTime:[model.reg_time intValue] andFormatter:@"yyyy-MM-dd HH:mm:ss"];
    ...
    if (model.email.length > 0) {
        self.typeLabel.text = model.email;
    }else{
        self.typeLabel.text = model.phone;
    }
    if ([model.pid_area_type isEqualToString:@"1"]) {
        self.quyu.text = @"A";
    }else{
        self.quyu.text = @"B";
    }
...
- (void)refreshWithModel2:(mlistModel *)model{
    ...
    self.endTime.text = [str1 lastObject];
    self.quyu.text = model.num;
    if ([model.mining_type isEqualToString:@"1"]) {
