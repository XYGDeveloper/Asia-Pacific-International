...
-(void)setupUI
{
    self.title = kLocat(@"kcurrentuantity_release_record");
    ...
    [_actionButton setTitle:[NSString stringWithFormat:@"%@%@",kLocat(@"k_popview_select_paywechat_edit_sure"),kLocat(@"Assert_detail_release")] forState:UIControlStateNormal];
    ...
    _lockLabel.text = [NSString stringWithFormat:@"%@:%@",kLocat(@"Assert_detail_lockvolume"),_dataDic[@"lock_num"]];;
...
-(void)checkIfReleaseToday
{
    ...
    [kNetwork_Tool POST_HTTPS:[Utilities handleAPIWith:@"/AccountManage/isReceive"] andParam:param completeBlock:^(BOOL success, NSDictionary *responseObj, NSError *error) {
        kHideHud;
        if (success) {
            //1为已领取，2为未领取
            ...
            double releaseVol = [_dataDic[@"lock_num"] doubleValue] * _rate / 100;
...
-(void)releaseAction
{
    ...
    param[@"currency_id"] = _currencyID;
    kShowHud;
    [kNetwork_Tool POST_HTTPS:[Utilities handleAPIWith:@"/AccountManage/everydayFreed"] andParam:param completeBlock:^(BOOL success, NSDictionary *responseObj, NSError *error) {
