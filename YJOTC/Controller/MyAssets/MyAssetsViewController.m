...
@interface MyAssetsViewController ()<UITableViewDelegate,UITableViewDataSource,ApiRequestDelegate>
...
//    [self lodata];
    ...
    self.api  = [[GetAssetListApi alloc]initWithKey:kUserInfo.token lanage:[Utilities gadgeLanguage:@""] token_id:[NSString stringWithFormat:@"%ld",kUserInfo.uid]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    MyAssetDetailViewController *asset = [MyAssetDetailViewController new];
    asset.avaStr = self.model.sum.usd;
    asset.frenStr = self.model.allmoneys;
    asset.current_id = model.currency_id;
    asset.type = model.currency_name;
    kNavPush(asset);
}
...
//    sumModel *model = [sumModel mj_objectWithKeyValues:self.model.sum];
    ...
    self.head.tradeConutDetail.text = [NSString stringWithFormat:@"≈%@USD",self.model.sum.usd];
    ...
    if (self.ishidden == YES) {
        ...
        for (current_userModel *model in self.model.currency_user) {
            if ([model.sum_award floatValue]> 0) {
                [arr addObject:model];
            }
        }
