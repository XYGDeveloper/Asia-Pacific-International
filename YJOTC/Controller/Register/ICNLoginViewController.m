...
@interface ICNLoginViewController () <NTESVerifyCodeManagerDelegate>
...
//    [registerLabel addGestureRecognizer:[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(registerAction)]];
    ...
    [button addTarget:self action:@selector(showVerifyInfo) forControlEvents:UIControlEventTouchUpInside];
...
-(void)loginActionWithValidate:(NSString *)validate
{
    [self hideKeyBoard];
...
-(NTESVerifyCodeManager *)manager {
    if (!_manager) {
        ...
        if ([currentLanguage containsString:@"en"]) {//英文
            _manager.lang = NTESVerifyCodeLangEN;
        }else if ([currentLanguage containsString:@"Hant"]){//繁体
            _manager.lang = NTESVerifyCodeLangCN;
        }else if ([currentLanguage containsString:Korean]){
            _manager.lang = NTESVerifyCodeLangKR;
        }else{
            _manager.lang = NTESVerifyCodeLangJP;
        }
                         ...
                         message:(NSString *)message
{
    ...
    if (result == YES) {
        [self loginActionWithValidate:validate];
    }else{
        [self showTips:@"验证码错误"];
    }
}
