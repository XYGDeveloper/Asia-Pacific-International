...
//    _tableView.tableHeaderView = [self creatHeaderView];
    ...
    [label addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithActionBlock:^(id  _Nonnull sender) {
        ...
        BaseWebViewController *vc = [[BaseWebViewController alloc] initWithWebViewType:BaseWebVCWebViewTypeFullScreen title:@"" urlString:[NSString stringWithFormat:@"%@/Mobile/Art/details/team_id/154.html.html",kBasePath]];
...
//        cell.itemLabel.text = self.phoneArray[indexPath.row];
        if (indexPath.row == 0) {
            ...
            cell.icon.image = kImageFromStr(@"yanzhengma");
            cell.TF.text = @"110000";//客户需求，默认设置110000
            _inviteTF = cell.TF;
        }
